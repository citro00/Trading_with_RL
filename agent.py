...
class Agent:
    ...
    def new_train(self, iterations, checkpoint, budget, env:gym.Env):
        ...
        for i in range(iterations):
            ...
            state_formatted = ut.state_formatter(state)
            #print(state_formatted)

            #iteration_seeds.append((i, seed))
            starting_money = budget

            for t in range(0, len(self.trend)-1, self.skip):
                action = self.act(state_formatted)
                if action == 1:
                    obseravtion, env_reward, terminated, truncated, info = env.step(1)
                elif action == 2:
                    obseravtion, env_reward, terminated, truncated, info = env.step(0)
                else: 
                    obseravtion, env_reward, terminated, truncated, info = env.step(1)

                #print(f"Observation: {obesravtion}")
                #print(f"Type of observation: {type(obesravtion)}")
                actual_price = self.trend[t]
                if action == 1 and starting_money >= actual_price and t < (len(self.trend) - self.half_window):
                    '''stocks_quantity = round(starting_money/self.trend[t])
                    inventory.append(tuple(self.trend[t], stocks_quantity))
                    starting_money -= (self.trend[t]*stocks_quantity)'''
                ...
                observation_formatted = ut.state_formatter(obseravtion)
                ...
                try: 
                    ...
                    print(f"Observation_foromatted: {observation_formatted}")
                    print(f"Observation: {obseravtion}")
            if (i+1) % checkpoint == 0:
                print('epoch: %d, total rewards: %f.3, loss: %f, total money: %f' % (i + 1, total_profit, loss, starting_money))